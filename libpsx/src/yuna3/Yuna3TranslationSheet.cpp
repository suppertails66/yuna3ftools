...
void Yuna3TranslationSheet::addStringEntry(std::string id, std::string content,
                    std::string prefix, std::string suffix,
                    std::string translationPlaceholder) {
  Yuna3TranslationSheetEntry entry;
  entry.type = Yuna3TranslationSheetEntry::type_string;
  entry.stringId = id;
...
void Yuna3TranslationSheet::addCommentEntry(std::string comment) {
  Yuna3TranslationSheetEntry entry;
  entry.type = Yuna3TranslationSheetEntry::type_comment;
  entry.stringContent
    = std::string("//==================================\n")
      + "// " + comment
      + std::string("\n//==================================");
...
void Yuna3TranslationSheet::addSmallCommentEntry(std::string comment) {
  Yuna3TranslationSheetEntry entry;
  entry.type = Yuna3TranslationSheetEntry::type_comment;
  entry.stringContent
    = std::string("//=====\n")
...
void Yuna3TranslationSheet::addMarkerEntry(std::string content) {
  Yuna3TranslationSheetEntry entry;
  entry.type = Yuna3TranslationSheetEntry::type_marker;
  entry.stringId = content;
  entries.push_back(entry);
}

bool Yuna3TranslationSheet::hasStringEntryForId(std::string id) const {
  return (stringEntryMap.find(id) != stringEntryMap.end());
}

const Yuna3TranslationSheetEntry& Yuna3TranslationSheet
    ::getStringEntryById(std::string id) const {
  return stringEntryMap.at(id);
}

void Yuna3TranslationSheet::importCsv(std::string filename) {
  TCsv csv;
  {
    TBufStream ifs;
...
void Yuna3TranslationSheet::exportCsv(std::string filename,
                      Yuna3TranslationSheet::LinkData* linkData) const {
  ...
  for (Yuna3TranslationSheetEntryCollection::const_iterator it
        = entries.cbegin();
       it != entries.cend();
       ++it) {
    const Yuna3TranslationSheetEntry& entry = *it;
    switch (entry.type) {
    case Yuna3TranslationSheetEntry::type_string:
    {
      // col A = type
      ofs << "\"string\"";
...
//          << "\"";
      ...
      break;
    case Yuna3TranslationSheetEntry::type_comment:
    {
...
//      ofs << "\"" << entry.stringContent << "\"";
      ...
      break;
    case Yuna3TranslationSheetEntry::type_marker:
    {
      if (entry.stringId.compare("dict_section_start") == 0) {
        ...
        throw TGenericException(T_SRCANDLINE,
                                "Yuna3TranslationSheet::exportCsv()",
                                "unknown marker: "
                                  + entry.stringId);
      }
    }
...
int Yuna3TranslationSheet::numEntries() const {
  return entries.size();
}

void Yuna3TranslationSheet::addBoxDuplicates(
    std::map<std::string, std::string>& boxMap,
    std::string content,
    std::string cell) {
...
std::map<std::string, std::string>::iterator Yuna3TranslationSheet
  ::findBoxDuplicate(
    std::map<std::string, std::string>& boxMap,
    std::string content) {
  std::vector<std::string> boxes = splitByBoxes(content);
...
std::vector<std::string> Yuna3TranslationSheet
...
std::string Yuna3TranslationSheet::getDictionariedString(LinkData& linkData,
                                         std::string content) {
  std::string result;
...
std::vector<std::string> Yuna3TranslationSheet::splitByDictEntries(
...
std::string Yuna3TranslationSheet::getQuoteSubbedString(std::string content) {
...
Yuna3TranslationSheet::LinkData::LinkData()
  : linked(false) { }
