#ifndef YUNA3LINEWRAPPER_H
#define YUNA3LINEWRAPPER_H
...
class Yuna3LineWrapper : public BlackT::TLineWrapper {
public:
  struct CharSizeTableEntry {
    int glyphWidth;
    int advanceWidth;
  };
  
  typedef std::map<int, CharSizeTableEntry> CharSizeTable;
  typedef BlackT::TTwoDArray<char> KerningMatrix;

  Yuna3LineWrapper(BlackT::TStream& src__,
                  ResultCollection& dst__,
                  const BlackT::TThingyTable& thingy__,
                  CharSizeTable sizeTable__,
                  CharSizeTable sizeTableEmph__ = CharSizeTable(),
                  KerningMatrix kerningMatrix__ = KerningMatrix(),
                  KerningMatrix kerningMatrixEmph__ = KerningMatrix(),
                  bool isNonstd = false,
                  int xSize__ = -1,
                  int ySize__ = -1,
...
//  const static int default_controlOpsStart = 0xEF;
  const static int default_controlOpsStart = 0x100;
