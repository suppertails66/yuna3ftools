...
#define YUNA3READER_IGNORE_BAD_INPUT 0
...
Yuna3ScriptReader::ResultString::ResultString()
  : originalOffset(-1),
    originalSize(-1),
    initialized(false),
    autoStarted(false),
...
Yuna3ScriptReader::ResultRegion::ResultRegion()
  : regionType(regionType_none),
    soundCount(0),
    choiceCount(0) { }

Yuna3ScriptReader::Yuna3ScriptReader(
...
bool Yuna3ScriptReader::operator()() {
  try {
    while (!src.eof()) {
      std::string line;
      src.getLine(line);
...
void Yuna3ScriptReader::loadThingy(const BlackT::TThingyTable& thingy__) {
  thingy = thingy__;
}
  
void Yuna3ScriptReader::outputNextSymbol(TStream& ifs) {
...
/*    std::cerr << currentResult.id << ":"
      << " " << soundStartTime
      << " " << lastWaitTime
      << " " << autoOnTime
      << std::endl;*/
    int timeAsInt = autoOnTime * framesPerSecond;
...
//    }  
  ...
  #if YUNA3READER_IGNORE_BAD_INPUT
...
//  if (ifs.eof()) return;
  ...
  throw TGenericException(T_SRCANDLINE,
                          "Yuna3ScriptReader::outputNextSymbol()",
                          "Line "
...
void Yuna3ScriptReader::flushActiveScript() {
...
//  terminateCurrentStringIfActive();
  ...
  if (
//      sceneModeOn
      (currentResult.hasProperty("endTime"))
...
//    std::cerr << currentResult.properties.size() << std::endl;
    ...
    double soundEndTime = TStringConversion::fromString<double>(
      currentResult.properties.at("endTime"));
...
void Yuna3ScriptReader::resetScriptBuffer() {
...
void Yuna3ScriptReader::terminateCurrentStringIfActive() {
  if (stringIsActive) {
    outputTerminateCommand();
    stringIsActive = false;
  }
...
void Yuna3ScriptReader::outputTerminateCommand() {
  int opcode = thingy.matchTableEntry("[null]").id;
  currentScriptBuffer.writeu8(opcode);
}

void Yuna3ScriptReader::outputOffCommand() {
  terminateCurrentStringIfActive();
...
void Yuna3ScriptReader::outputWaitCommand(double secTime) {
  terminateCurrentStringIfActive();
  ...
  currentScriptBuffer.writeu8(opcode);
  currentScriptBuffer.writeu16le(frameValue * timeScale);
  
  lastWaitTime = secTime;
}

void Yuna3ScriptReader::outputVramOverrideCommand(int x, int y) {
  terminateCurrentStringIfActive();
  
  int opcode = thingy.matchTableEntry("[vramoverride]").id;
  currentScriptBuffer.writeu8(opcode);
  currentScriptBuffer.writeu16le(x);
  currentScriptBuffer.writeu16le(y);
}

void Yuna3ScriptReader::outputSlotCommand(int slotNum) {
  terminateCurrentStringIfActive();
...
void Yuna3ScriptReader::outputPalCommand(int palNum) {
  terminateCurrentStringIfActive();
...
void Yuna3ScriptReader
  ...
  currentScriptBuffer.writeu8(opcode);
  currentScriptBuffer.writeu16le(frameValue1 * timeScale);
  currentScriptBuffer.writeu16le(frameValue2 * timeScale);
}
...
void Yuna3ScriptReader
  ...
  currentScriptBuffer.writeu16le(targetValue);
  currentScriptBuffer.writeu16le(frameTime * timeScale);
}
...
void Yuna3ScriptReader::processDirective(BlackT::TStream& ifs) {
...
//    processStartMsg(ifs);
    ...
    if (name.compare("area") == 0) {
      ...
      throw TGenericException(T_SRCANDLINE,
                              "Yuna3ScriptReader::processDirective()",
                              "Line "
                                + TStringConversion::intToString(lineNum)
...
/*  else if (name.compare("SETFILE") == 0) {
  ...
  else if (name.compare("VRAMOVERRIDE") == 0) {
    int x = TParse::matchInt(ifs);
    TParse::matchChar(ifs, ',');
    int y = TParse::matchInt(ifs);
    outputVramOverrideCommand(x, y);
  }
...
void Yuna3ScriptReader::processLoadTable(BlackT::TStream& ifs) {
  std::string tableName = TParse::matchString(ifs);
  TThingyTable table(tableName);
  loadThingy(table);
}

void Yuna3ScriptReader::processStartMsg(BlackT::TStream& ifs) {
/*  currentScriptSrcOffset = TParse::matchInt(ifs);
  TParse::matchChar(ifs, ',');
...
void Yuna3ScriptReader::processEndMsg(BlackT::TStream& ifs) {
  flushActiveScript();
}

void Yuna3ScriptReader::processIncBin(BlackT::TStream& ifs) {
...
void Yuna3ScriptReader::processBreak(BlackT::TStream& ifs) {
  breakTriggered = true;
}

void Yuna3ScriptReader::processStartRegion(BlackT::TStream& ifs) {
  flushActiveScript();
...
void Yuna3ScriptReader::processEndRegion(BlackT::TStream& ifs) {
//  TParse::matchInt(ifs);
//  TParse::matchInt(ifs);
////  flushActiveScript();
//  breakTriggered = true;
...
/*void Yuna3ScriptReader::processSetIndexList(BlackT::TStream& ifs) {
  currentScriptIndices.clear();
...
double Yuna3ScriptReader::matchTime(BlackT::TStream& ifs) {
