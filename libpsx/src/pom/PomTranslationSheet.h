#ifndef POMTRANSLATIONSHEET_H
#define POMTRANSLATIONSHEET_H
...
class PomTranslationSheetEntry {
public:
  enum Type {
    type_none,
    type_string,
...
typedef std::vector<PomTranslationSheetEntry>
  PomTranslationSheetEntryCollection;

class PomTranslationSheet {
protected:
  struct DictEntry {
...
public:
  ...
  const PomTranslationSheetEntry& getStringEntryById(std::string id) const;
...
protected:
  ...
  PomTranslationSheetEntryCollection entries;
  std::map<std::string, PomTranslationSheetEntry> stringEntryMap;
