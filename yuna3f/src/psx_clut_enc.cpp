...
int main(int argc, char* argv[]) {
  if (argc < 3) {
    cout << "PlayStation CLUT to GPU address encoder" << endl;
...
//  int raw = TStringConversion::stringToInt(std::string(argv[1]));
  int x = TStringConversion::stringToInt(std::string(argv[1]));
  int y = TStringConversion::stringToInt(std::string(argv[2]));
...
//  int addr = (y * 0x800) + x;
  ...
  int addr = (y * 0x800) + x;
  int encoded = (x / 32) | (y << 6);
  
  std::cout << "gpu addr = " << std::hex << addr
            << ", encoded = " << std::hex << encoded
    << std::endl;
